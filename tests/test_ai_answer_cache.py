...
from study_qb_assistant.ai_answer_cache import AIAnswerCache, cache_key  # noqa: E402
from study_qb_assistant.models import QuestionQuery  # noqa: E402


class AIAnswerCachePersistenceTests(unittest.TestCase):
    """验证 AI 答案使用统一题库 JSONL 持久化。"""

    def test_legacy_json_cache_is_migrated_to_ai_learned_jsonl(self) -> None:
        """旧版 entries JSON 缓存应迁移为带 AI 标签的标准题库记录。"""
        query = QuestionQuery(
            title="单选题(1分)旧缓存迁移题",
            options=("正确项", "干扰项"),
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            cache = AIAnswerCache(learned_path, legacy_paths=(legacy_path,))
