...
def test_process_and_save_brand_logo(temp_brand_dir):
    ...
    assert urls["original"] == "/api/v1/media/brand/logo_original.png"
    assert urls["lg"] == "/api/v1/media/brand/logo_lg.png"
    assert urls["md"] == "/api/v1/media/brand/logo_md.png"
    assert urls["sm"] == "/api/v1/media/brand/logo_sm.png"
...
def test_system_logo_upload_and_download(temp_brand_dir, temp_db_dir, monkeypatch):
    ...
    # 1. 注册超级管理员以获取 Token
    client.post(
        "/api/v1/auth/register", json={"username": "sysadmin", "password": "securepassword"}
    )
    login_res = client.post(
        "/api/v1/auth/login", json={"username": "sysadmin", "password": "securepassword"}
    )
    ...
    # 4. 请求下载获取裁剪后的媒体文件
    download_res = client.get("/api/v1/media/brand/logo_lg.png")
    ...
    traversal_res = client.get("/api/v1/media/brand/../ocs/images/somefile.png")
    assert traversal_res.status_code == 404

    traversal_res_2 = client.get("/api/v1/media/brand/sub/file.png")
