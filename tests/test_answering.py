...
from study_qb_assistant.ai_answer_cache import AIAnswerCache  # noqa: E402
...
class AnswerServiceTests(unittest.TestCase):
    ...
    def test_model_fallback_is_labeled_for_review(self) -> None:
        ...
        # 输入一道本地必定不存在的题目
        result = service.query(QuestionQuery(title="一道本地题库里不存在的题目", question_type="single"))
    ...
    def test_ai_cache_promotes_after_repeated_high_confidence_answer(self) -> None:
        """测试多次重复的高置信度模型回答是否会将该条目晋升并存入 AI 缓存中。
        ...
        self.assertEqual(first.debug["ai_cache_status"], "pending")
        
        # 第二次查询：达到确认次数限制，触发晋升至 trusted，并写入本地缓存文件
        self.assertEqual(second.resolution_mode, "llm_fallback")
        self.assertEqual(second.debug["ai_cache_status"], "trusted")
        
        # 第三次查询：直接读取 AI 缓存，不调用模型
    ...
    def test_trusted_ai_cache_direct_hit_uses_ai_question_bank_source(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            cache = AIAnswerCache(Path(temp_dir) / "ai-learned.jsonl", min_confidence=0.95, min_confirmations=1)
            query = QuestionQuery(
                title="单选题(1分)直接命中AI题库",
                options=("复用答案", "干扰项"),
                question_type="single",
            )
            cache.record_model_answer(query, provider.answer(query), provider_name=provider.provider_name)
            service = AnswerService(
                LocalQuestionIndex(()),
                model_provider=provider,
                allow_model_fallback=True,
                ai_answer_cache=cache,
            )
    ...
    def test_ai_cache_does_not_promote_conflicting_answers(self) -> None:
        ...
        self.assertEqual(second.debug["ai_cache_status"], "conflict")
        # 第三次：即使模型重回正确答案 A，由于之前已经记录过冲突，状态信号仍为 conflict
        self.assertEqual(third.debug["ai_cache_status"], "conflict")
    ...
    def test_ai_cache_skips_internally_inconsistent_answer(self) -> None:
        ...
        self.assertEqual(first.debug["ai_cache_status"], "unsafe_not_cached")
        # 第二次：模型给出了合规安全的答案，重新转入 pending 状态进行多轮确认
        self.assertEqual(second.debug["ai_cache_status"], "pending")
        self.assertEqual(provider.calls, 2)
...
def _service_with_cache(provider: SequenceModelProvider, cache_path: Path) -> AnswerService:
    ...
    return AnswerService(
        ...
        model_provider=provider,
        allow_model_fallback=True,
        ai_answer_cache=AIAnswerCache(cache_path, min_confidence=0.95, min_confirmations=2),
    )
