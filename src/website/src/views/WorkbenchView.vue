...
import { dashboardApi, tokenApi } from '@/api/endpoints'
import { ApiException, getApiTokenSecret } from '@/api/http'
import type { ApiToken, OcsConfig, Workbench } from '@/api/types'
...
async function handleCopyImportScript(tokenId?: string) {
  try {
...
async function confirmTokenSelection() {
  ...
  await handleCopyImportScript(selectedTokenId.value)
}
