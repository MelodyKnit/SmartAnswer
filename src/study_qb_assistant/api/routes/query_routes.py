...
from ..context import get_auth_service, get_lookup_service, get_platform_service, guard_protected_request
...
def build_query_router() -> APIRouter:
    ...
    def ocs_config(request: Request) -> JSONResponse:
        ...
        return JSONResponse(build_ocs_config(f"http://{host}"))
    ...
    def query_get(
        ...
        query = build_query_from_payload(
            QueryPayload(
                title=title,
                options=sanitize_query_options(title, question_type or "unknown", split_options(options)),
                type=question_type,
    ...
    def ocs_query_post(request: Request, payload: QueryPayload = Body(default_factory=QueryPayload)) -> JSONResponse:
        denied = guard_protected_request(request)
        if denied:
            return denied
        return run_lookup(
