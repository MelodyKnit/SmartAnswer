...
该模块定义了 AnswerService 类，将本地题库检索、内置匹配规则、AI 自动沉淀题库和 LLM 提供商 fallback 流程串联起来。
"""

from __future__ import annotations

from .ai_answer_cache import AIAnswerCache, CachedAIAnswer
...
class AnswerService:
    ...
    def __init__(
        ...
        *,
        ...
        ai_answer_cache: AIAnswerCache | None = None,
    ) -> None:
        """初始化答案决议服务。
    ...
    def query(self, query: QuestionQuery) -> QueryResult:
        ...
        3. 已有的受信任 AI 自动沉淀题库记录 (CachedAIAnswer)
        4. 本地模糊匹配
        ...
        # 步骤 3：尝试匹配已经过多次验证的受信任 AI 自动沉淀记录
        cached_answer = self.ai_answer_cache.get_trusted(query) if self.ai_answer_cache else None
        ...
        if self.allow_model_fallback and self.model_provider and local_result.error_code == "NOT_FOUND":
    ...
    def status(self) -> dict:
        ...
        Returns:
            dict: 包含索引状态、模型提供商状态 and AI 缓存配置的字典。
        """
        ...
        cache_status = (
            self.ai_answer_cache.status()
            if self.ai_answer_cache is not None
            else {"enabled": False}
        )
        return {
            "lookup": self.index.status(),
            "model": model_status,
            "ai_answer_cache": cache_status,
        }
    ...
    def _model_fallback(self, query: QuestionQuery) -> QueryResult:
        ...
        if self.ai_answer_cache is not None and is_cache_safe_answer(query, model_answer):
            cache_entry = self.ai_answer_cache.record_model_answer(
                query,
                model_answer,
        ...
        elif self.ai_answer_cache is not None:
    ...
    def _result_from_cached_answer(
        self,
        query: QuestionQuery,
        cached_answer: CachedAIAnswer,
    ) -> QueryResult:
        """从受信任的 AI 自动沉淀题库条目中封装标准 QueryResult。"""
        ...
        return QueryResult(
            ...
            debug={
                ...
                "ai_cache_status": cached_answer.status,
