...
def build_model_query(query: QuestionQuery) -> QuestionQuery:
    """为视觉模型补齐可访问的本地图床图片 URL。"""

    from .question_images import hydrate_query_images_for_model

    return hydrate_query_images_for_model(query)
...
def load_query_image_assets(query: QuestionQuery) -> tuple[ImageAsset, ...]:
    ...
    seen: set[str] = set()
    stem_data_urls = normalize_image_data_urls(query.image_data_urls)
    option_data_labels = {
        str(label or "").strip().upper()
        for label, data_url in query.option_image_data_urls.items()
        if normalize_image_data_urls((data_url,))
    }
    data_urls = normalize_image_data_urls(
        stem_data_urls,
    ...
    url_groups: list[object] = []
    if not stem_data_urls:
        url_groups.extend((query.image_urls, (query.title,)))
    url_groups.append(
        tuple(
            url
            for label, url in query.option_image_urls.items()
            if str(label or "").strip().upper() not in option_data_labels
        )
    )
    for url in normalize_image_urls(*url_groups):
        if url in seen:
            continue
