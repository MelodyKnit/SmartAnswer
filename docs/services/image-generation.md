...
Gemini 模型若以 OpenAI 兼容聊天接口接入，必须使用旧 `openai-chat-image` 协议；该兼容协议没有
图片输入字段，因此只能提供文生图。需要图文生图、局部编辑或多图参考时，管理员应在生图模型配置
中显式切换为 `gemini-native`，并填写供应商提供的 Gemini `generateContent` API 基础地址。该地址通常
使用与 OpenAI 兼容 `/v1` 不同的版本路径，例如供应商文档声明的 `/v1beta`。

保存 Gemini 原生配置后，先选择经实际验证的画幅比例、像素档位和鉴权方式，再分别执行整图编辑、
局部编辑及多图参考能力测试。系统只会向用户开放当前配置指纹下测试通过的模式；不会根据模型名称、
Base URL 或旧聊天模型配置自动推断或迁移协议，避免把不兼容的上游请求误发到生产模型。
