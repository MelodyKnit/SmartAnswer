...
## 文档元信息
*   **系统名称：** 题库与检索增强大模型答题助手 (StudyQuestionBankAssistant)
*   **网站名称：** 智慧答题 (SmartAnswer)
*   **版本号：** V1.0.0
*   **编写日期：** 2026-06-07
...
系统基于 FastAPI + uvicorn 的轻量化 ASGI HTTP 服务构建，前端控制台采用纯 HTML5 + 现代 CSS（简约扁平化设计）+ 原生 JS 驱动，实现了高内聚低耦合的决策结构：
...
| 配色令牌名称 | 对应色值 | 界面应用场景及心理学考量 |
| :--- | :--- | :--- |
| **浅白灰背景 (Bg Main)** | `#f8fafc` | 控制台的主背景色。高明度、低饱和度的 Slate 灰，显得极其干净、理性，利于长时间注视文字。 |
| **纯白容器背景 (Card Bg)**| `#ffffff` | 所有模块卡片与侧边栏的底色。提供极高的内容反差度，完全依赖阴影实现层级。 |
| **极细边框线 (Border)** | `#e2e8f0` (Slate-200) | 用于分割不同的信息模块与卡片边缘。替代阴影作为边界区分，贯彻纯平线条美学。 |
| **靛蓝主标色 (Indigo)** | `#4f46e5` | 按钮、活动导航栏、主焦点标签色。冷调深蓝，象征智慧、科技与秩序，且对比度极高，易于视线快速定位。 |
| **青绿辅助色 (Teal)** | `#0d9488` | 图表比重对比点缀、二级辅助按钮颜色。 |
| **状态绿 (Success)** | `#16a34a` (文字), `#f0fdf4` (底) | 代表“本地精准命中”或“服务健康”。使用低纯度草绿底衬，减少亮绿的刺激感。 |
| **状态橙 (Warning)** | `#d97706` (文字), `#fef3c7` (底) | 代表“待审核项”或“Fallback 生成”，提示需要人工介入。 |

#### B. Spacing 间距与扁平布局
*   **极致扁平按钮：** 边角弧度限制在 `6px`，背景为扁平单色（如纯 Indigo-600），悬停时仅进行深浅平滑切换（`#4f46e5` $\rightarrow$ `#4338ca`），无渐变和发光。
*   **微小硬投影：** 不使用高扩散毛玻璃投影。只使用最微弱的硬阴影 `box-shadow: 0 1px 2px rgba(0,0,0,0.05)`，保持卡片与白底的高级感。
*   **8px 简约栅格步进：** 布局距离统一以 `8px` 为单位阶梯（`16px` 标签边距，`24px` 卡片 Padding，`40px` 控制台主屏 Margin），大面积空旷留白使得界面呼吸感极强。
