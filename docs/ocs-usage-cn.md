...
本项目已经整理成“本地题库服务 + 可切换大模型 Provider + OCS 风格配置”的结构。
...
OCS 配置
  ...
  -> 仍未命中时可选调用 OpenAI-compatible 大模型 API
...
模型不绑定 Ollama。只要服务端配置的是 OpenAI-compatible `/chat/completions` 接口，可以接本地 Ollama、LM Studio、vLLM，也可以接 DeepSeek、通义千问、智谱、OpenAI-compatible 网关等云 API。
...
不要把 API Key 写进 OCS 配置。推荐放在本地服务环境变量里。
...
- ClassBot/New API 这类网关按 OpenAI-compatible Chat Completions 接入；本项目默认使用流式响应并解析 `choices[].delta.content`。
- 单选题会返回 `A`、`B` 等字母；多选题会返回 OCS 更容易识别的 `A#B#C` 格式。

默认联网搜索增强不需要 key，使用 DuckDuckGo Instant Answer：
...
搜索增强默认使用 `duckduckgo`，不需要 key；如果网络不稳定或不想联网，可设置 `STQB_WEB_SEARCH_PROVIDER="none"`。本地题库仍然优先；本地没命中时先调用搜索 API 获取证据片段，再把证据和题目一起交给大模型作答，用于降低模型凭记忆幻觉。搜索失败会进入短期冷却，避免每道题都卡在同一个不可达搜索源上。
...
AI 学习结果默认写入 `data\normalized\ai-learned.jsonl`，格式与普通题库一致，来源标为 `AIGenerated`，并带有 `ai_generated`、`auto_learned`、`status:*` 等标签。第一次高置信模型答案会先记为 `pending`，同题同选项后续再次得到相同答案才晋升为 `trusted`；只有 `trusted` 才会进入本地检索并以 `resolution_mode: ai_cache` 返回。若模型对同一题给出冲突答案，该条记录会进入 `conflict`，不会被用于自动答题。旧版 `data\runtime\ai-answer-cache.json` 会在启用模型学习时作为兼容来源迁移。

项目还支持一个可选的种子规则文件，但默认不启用，只有显式配置 `STQB_ANSWER_RULES_PATH` 时才会读取：

```text
configs\my-answer-rules.json
```

它只用于放少量高确定性的固定表述，不作为日常补题入口。后续遇到新题，默认应依靠“模型作答 + 一致性校验 + AI 题库沉淀”自动积累，而不是不断往代码或规则 JSON 里人工追加题目。
...
如果使用 LM Studio、vLLM 或其他本地服务，把 `STQB_LLM_BASE_URL` 和 `STQB_LLM_MODEL` 改成对应服务即可。
...
```powershell
python scripts\serve_local.py --index data\normalized\verified.jsonl --host 127.0.0.1 --port 8765 --llm-fallback
```
...
后续如果找到明确授权、公开契约、稳定可验证的题库 API，可以按 `External source adapter` 的方式接入，不影响现有 OCS 配置。
